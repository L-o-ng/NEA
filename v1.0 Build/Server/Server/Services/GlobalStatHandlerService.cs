    ...
    public class GlobalStatHandlerService : GlobalStatHandler.GlobalStatHandlerBase
    {
        ...
        public override Task<Empty> UploadTime(Time request, ServerCallContext context) {
            ...
            using (SQLiteConnection conn = new("Data Source= mazeData.db; Version = 3; New = True; Compress = True; ")) {
                ...
                cmd.CommandText = @"SELECT Time1Name, Time2Name, Time3Name, Time4Name, Time5Name, Time6Name, Time7Name, Time8Name, Time9Name, Time10Name
                                    FROM GlobalStats";
                ...
                for (int i = 0; i < 10; i++) {
                    cmd.CommandText = $"UPDATE GlobalStats SET Time{i + 1}Milliseconds = {milliseconds[i]}";
                    cmd.ExecuteNonQuery();
                    cmd.CommandText = $"UPDATE GlobalStats SET Time{i + 1}Display = '{displayTimes[i]}'";
                    cmd.ExecuteNonQuery();
                    cmd.CommandText = $"UPDATE GlobalStats SET Time{i + 1}Name = '{usernames[i]}'";
                    cmd.ExecuteNonQuery();
                }
            }
            return Task.FromResult(new Empty());
